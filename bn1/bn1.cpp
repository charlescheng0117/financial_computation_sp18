...
double CRRBinomial_am_call(double sigma);
double CRRBinomial_am_put(double sigma); 
...
int main(int argc, char const *argv[])
{
	...
	*/
    ...
    if (criterion[0] == 'B') {
        ...
        printf("European call: %lf\n", bisection(BS, CALL, EU));
        printf("European put:  %lf\n", bisection(BS, PUT, EU));
        printf("Binomial tree\n");
        printf("European call: %lf\n", bisection(BINOM, CALL, EU));
        printf("European put:  %lf\n", bisection(BINOM, PUT, EU));
        printf("American call: %lf\n", bisection(BINOM, CALL, AM));
        printf("American put:  %lf\n", bisection(BINOM, PUT, AM));
    } else {
        ;
    }
	return 0;
...
double CRRBinomial_am_call(double sigma) { // n: # of periods
...
double CRRBinomial_am_put(double sigma) { // n: # of periods
...
double bisection(int model, int option_t, int eu_na) {
    ...
    if (model == BS) {
        if (option_t == CALL) {
            ans = bisection_helper(callBlackScholes, option_price);

        } else { //PUT
            ans = bisection_helper(putBlackScholes, option_price);
        }
    } else { // Binomial tree
        if (option_t == CALL) {
            if (eu_na == EU) {
                ans = bisection_helper(combinatorial_binom_call, option_price);
            } else {
                ans = bisection_helper(CRRBinomial_am_call, option_price);
            }
        } else { // PUT
            if (eu_na == EU) {
                ans = bisection_helper(combinatorial_binom_put, option_price);
            } else {
                ans = bisection_helper(CRRBinomial_am_put, option_price);
            }
        }
    }
    // BS, eu, call
